...
public class CustomerDAOImpl implements CustomerDAO {
	...
	public List<Customer> getCustomers() {
		...
		/*
		 * String hql = "FROM Customer"; Query query = currentSession.createQuery(hql);
		 */

		List<Customer> customers = null;
		try {
			Query<Customer> theQuery = currentSession.createQuery("from Customer ORDER BY customerName",
					Customer.class);
			customers = theQuery.getResultList();

		} catch (IllegalArgumentException illegalArgumentException) {
			System.out.println(
					"The select list of the query is not assignable to the resultclass argument you have given");
		}
